...
namespace Tetris
{
    public class ConsoleRender
    {
        ...
        public void DrawInfo(int column, ILogic state, ScoreManager manager)
        {
            ...
            this.Write("Уровень:", column, 1);
            this.Write($"{state.Level} / 10", column, 2);

            this.Write("Очков:", column, 4);
            this.Write(manager.Score.ToString(), column, 5);

            this.Write("Лучшее:", column, 7);
            this.Write(manager.HighScore.ToString(), column, 8);

            this.Write("Управлять:", column, 10);
            this.Write("   ^", column, 11);
            this.Write(" <   >", column, 12);
            this.Write("   V", column, 13);
            this.Write(" Пробел", column, 14);
        }
        ...
        public void DrawGameOver(int score)
        {
            ...
            Console.ForegroundColor = ConsoleColor.Red;
            this.Write("Конец", row + 2, col + 1);
            this.Write("Игры!", row + 3, col + 2);
            Console.ForegroundColor = ConsoleColor.Green;
            this.Write("Очков:", row + 2, col + 3);
